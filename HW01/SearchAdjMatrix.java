...
public class SearchAdjMatrix {
    ...
    /* Sets up the matrix and set the number of vertices in the graph */
    public SearchAdjMatrix(int n) {
	graph = new int[4039][4039];
       	nodes = n;
	load();
		
    }

    public SearchAdjMatrix(int n, int[][] g) {
	graph = g;
	nodes = n;
    }
    ...
    /* For debugging to see the adjacency matrix */
	...
	while (q.size() > 0) {
		...
		if (graph[curr][j] == 1 && seen[j] == 0 && !(q.contains(j))) {
		    q.add(j);
		}
	    }
	}
	...
	while (s.size() > 0) {
	    ...
	    for (int j = 0; j < graph[curr].length; j ++) {
		if (graph[curr][j] == 1 && seen[j] == 0 && !(s.contains(j))) {
		    s.push(j);
		}
	    }
	}
	...
	long loadTimes = 0;
	SearchAdjMatrix obj = null;

	for (int i = 0; i < 20; i ++) {
	
	    startTime = System.nanoTime();
	    obj = new SearchAdjMatrix(4039);
